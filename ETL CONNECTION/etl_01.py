...
from dotenv import load_dotenv     # its come's from python-dotenv liabrary

load_dotenv()         # its job is to read .env file and load variables from .env file 


#we are storing connection details to variable conn we can give diff name to it..it just a object to store the connection details
